...
def direction_to_side(direction: str) -> str:
    ...
    if direction == "long":
        ...
        return "buy"  # Default to buy for unknown directions
...
def validate_short_trading_allowed(config: dict, side: str) -> tuple[bool, str]:
    """Validate if short trading is allowed for the given side."""
    if side == "sell" and not config.get("allow_short", False):
        return False, "Short selling disabled in configuration"
    
    # Check margin requirements for shorts
    if side == "sell":
        max_short_exposure = config.get("risk", {}).get("max_short_exposure", 0.3)
        current_short_exposure = 0.0
        
        # Calculate current short exposure (this would need to be implemented in position tracking)
        # For now, we'll just check the configuration
        
        if max_short_exposure <= 0:
            return False, "Maximum short exposure not configured"
    
    return True, "Short trading allowed"
...
def update_position_pnl(ctx):
    ...
    for sym, pos in ctx.positions.items():
        try:
            # Get the correct timeframe cache
...
def validate_paper_wallet_consistency(ctx):
    ...
    try:
        ...
        if balance_diff > 0.01:
            logger.warning(f"Balance inconsistency: ctx=${ctx.balance:.2f}, wallet=${ctx.paper_wallet.balance:.2f}")
            return False
        
        # Check position consistency
        ctx_symbols = set(ctx.positions.keys())
        wallet_symbols = set(ctx.paper_wallet.positions.keys())
        
        if ctx_symbols != wallet_symbols:
            missing_in_wallet = ctx_symbols - wallet_symbols
            missing_in_ctx = wallet_symbols - ctx_symbols
            if missing_in_wallet:
                logger.warning(f"Positions missing in wallet: {missing_in_wallet}")
            if missing_in_ctx:
                logger.warning(f"Positions missing in ctx: {missing_in_ctx}")
            return False
        
        # Validate individual position data
        for sym in ctx_symbols:
            ctx_pos = ctx.positions[sym]
            wallet_pos = ctx.paper_wallet.positions[sym]
            
            if (abs(ctx_pos["size"] - wallet_pos.get("size", wallet_pos.get("amount", 0))) > 0.0001 or
                ctx_pos["side"] != wallet_pos["side"]):
                logger.warning(f"Position data mismatch for {sym}")
                return False
        
        # Check for any negative balances
        if ctx.paper_wallet.balance < 0:
            logger.error(f"Negative paper wallet balance detected: ${ctx.paper_wallet.balance:.2f}")
            return False
...
def ensure_paper_wallet_sync(ctx):
    ...
    if ctx.config.get("execution_mode") == "dry_run" and ctx.paper_wallet:
        ...
        if status:
            logger.info(f"Paper wallet status: Balance=${status['balance']}, PnL=${status['realized_pnl']}, Win Rate={status['win_rate']}")
...
def get_paper_wallet_status(ctx):
    ...
    for sym, pos_ctx in ctx.positions.items():
        if sym in ctx.paper_wallet.positions:
            df = tf_cache.get(sym)
            if df is not None and not df.empty:
                current_price = float(df.iloc[-1]['close'])
...
async def execute_signals(ctx: BotContext) -> None:
    ...
    for candidate in results[:top_n]:
        ...
        try:
            ...
            # Use more conservative sentiment boost settings
            sentiment_boost = await get_lunarcrush_sentiment_boost(
                sym, 
                candidate["direction"],
                min_galaxy_score=70.0,  # Increased threshold
                min_sentiment=0.7       # Increased threshold
            )
            
            # Apply additional conservative factor to ensure sentiment is truly just a boost
            if sentiment_boost > 1.0:
                # Cap the boost to be more conservative
                sentiment_boost = 1.0 + (sentiment_boost - 1.0) * 0.5
                logger.info(f"Applied conservative sentiment boost {sentiment_boost:.3f} for {sym}")
            else:
                logger.debug(f"No sentiment boost applied for {sym}")
        ...
        # Validate short trading if this is a short position
        if side == "sell":
            allowed, reason = validate_short_trading_allowed(ctx.config, side)
            if not allowed:
...
async def handle_exits(ctx: BotContext) -> None:
    ...
    for sym, pos in list(ctx.positions.items()):
        ...
        pnl_pct = ((current_price - entry_price) / entry_price) * (
            1 if position_side == "buy" else -1
        )
        
        # Use enhanced risk manager to check for force exits first
        if ctx.risk_manager:
            # Assess momentum strength for risk manager
            momentum_strength = 0.5  # Default neutral
            try:
            ...
            if should_force_exit:
                logger.warning(
                    f"Risk manager forcing exit for {sym}: {exit_reason} at price {current_price:.6f}"
                )
                await _force_exit_position(ctx, sym, pos, current_price, exit_reason)
                continue
        
        # Update trailing stops using enhanced risk manager if available
        if ctx.risk_manager and hasattr(ctx.risk_manager, 'calculate_dynamic_trailing_stop'):
            # Use enhanced trailing stop calculation
            highest_price = pos.get("highest_price", entry_price)
            lowest_price = pos.get("lowest_price", entry_price)
            
            new_trailing_stop = ctx.risk_manager.calculate_dynamic_trailing_stop(
                sym, current_price, entry_price, position_side, highest_price, lowest_price
            )
            ...
            # Fallback to original trailing stop logic
            if pnl_pct >= ctx.config.get("exit_strategy", {}).get("min_gain_to_trail", 0):
                if position_side == "buy":  # Long position
                    ...
                    if current_price < pos.get("lowest_price", entry_price):
                        pos["lowest_price"] = current_price
                    pos["trailing_stop"] = pos["lowest_price"] * (1 + ctx.config.get("exit_strategy", {}).get("trailing_stop_pct", 0.02))
        
        # Check other exit conditions (trailing stops, take profits)
        ...
        # Update trailing stop if the exit manager suggests a better one
        if new_stop > 0:
            pos["trailing_stop"] = new_stop
        
        if exit_signal:
            await _force_exit_position(ctx, sym, pos, current_price, "trailing_stop_or_take_profit")

async def _force_exit_position(ctx: BotContext, sym: str, pos: dict, exit_price: float, reason: str):
    """Force exit a position with proper logging and cleanup."""
    logger.info(f"Exiting position {sym} due to {reason} at price {exit_price:.6f}")
    
    try:
        # Handle exit based on position type
        if sym.endswith("/USDC"):  # Solana position
            await _handle_solana_exit(ctx, sym, pos, exit_price, reason)
        else:  # CEX position
            await _handle_cex_exit(ctx, sym, pos, exit_price, reason)
            
    except Exception as e:
        logger.error(f"Failed to exit position {sym}: {e}")
        # Don't remove the position if exit failed - let it retry next cycle


async def _handle_solana_exit(ctx: BotContext, sym: str, pos: dict, exit_price: float, reason: str):
    """Handle Solana position exit."""
    try:
        # Close position in paper wallet first
        if ctx.config.get("execution_mode") == "dry_run" and ctx.paper_wallet:
        ...
        logger.info(f"Solana position closed: {sym}, PnL: ${pnl:.2f}")
        
    except Exception as e:
        logger.error(f"Error closing Solana position {sym}: {e}")


async def _handle_cex_exit(ctx: BotContext, sym: str, pos: dict, exit_price: float, reason: str):
    """Handle CEX position exit."""
    try:
        ...
        if ctx.config.get("execution_mode") == "dry_run" and ctx.paper_wallet:
            try:
                pnl = ctx.paper_wallet.close(sym, pos["size"], exit_price)
                ctx.balance = ctx.paper_wallet.balance
                logger.info(f"Paper trade closed: {pos['side']} {pos['size']} {sym} @ ${exit_price:.6f}, PnL: ${pnl:.2f}, balance: ${ctx.balance:.2f}")
        ...
        if sym in ctx.positions:
            ...
            if ctx.paper_wallet and sym in ctx.paper_wallet.positions:
                final_pnl = ctx.paper_wallet.unrealized(sym, exit_price)
                pos["pnl"] = final_pnl
        ...
        # Deallocate capital from risk manager
        if ctx.risk_manager:
        ...
        logger.info(f"Successfully exited CEX position {sym} due to {reason}")
        
    except Exception as e:
        logger.error(f"Error closing CEX position {sym}: {e}")
...
async def force_exit_all(ctx: BotContext) -> None:
    ...
    for sym, pos in list(ctx.positions.items()):
        ...
        # Use the unified exit function for both CEX and Solana
        await _force_exit_position(ctx, sym, pos, exit_price, "force_liquidation")
...
async def _monitor_micro_scalp_exit(ctx: BotContext, sym: str) -> None:
    ...
    # Use the unified exit function
    await _force_exit_position(ctx, sym, pos, exit_price, "micro_scalp_exit")
    ...
    while True:
        try:
            if state.get("running") and rotator.config.get("enabled"):
                # Use mock balance in dry run mode
                if config.get("execution_mode") == "dry_run":
                    current_balance = 10000.0  # Mock balance for dry run
                    holdings = {"USDT": 10000.0}  # Mock holdings for dry run
                else:
...
async def _main_impl() -> TelegramNotifier:
    ...
    try:
        # In dry run mode, use a mock balance instead of fetching from exchange
        if config.get("execution_mode") == "dry_run":
            init_bal = 10000.0  # Mock balance for dry run
            logger.info("Using mock balance %.2f for dry run mode", init_bal)
        else:
            if asyncio.iscoroutinefunction(getattr(exchange, "fetch_balance", None)):
                bal = await exchange.fetch_balance()
            else:
    ...
    try:
        while True:
            ...
            for token in detect_non_trade_tokens(balances):
                ...
                # Get balance - use mock data in dry run mode
                if config.get("execution_mode") == "dry_run":
                    bal_val = 10000.0  # Mock balance for dry run
                else:
                    if asyncio.iscoroutinefunction(
                        getattr(exchange, "fetch_balance", None)
                    ):
