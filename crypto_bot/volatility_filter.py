...
def calc_atr(df: pd.DataFrame, window: int = 14) -> float:
    ...
    try:
        if df.empty or len(df) < window + 1:
            logger.warning(f"DataFrame too short for ATR calculation: {len(df)} rows, need {window + 1}")
            return 0.0
        ...
        if recent.empty or recent["high"].isna().any() or recent["low"].isna().any() or recent["close"].isna().any():
            logger.warning("Invalid data in DataFrame for ATR calculation")
            return 0.0
        ...
        if tr.empty or len(tr) < window:
            logger.warning(f"Not enough valid TR values for ATR: {len(tr)} available, need {window}")
            return 0.0
            
        # Calculate ATR using rolling mean
        atr_series = tr.rolling(window, min_periods=1).mean()
        
        # Cache the result
        cached = cache_series(f"atr_{window}", df, atr_series, lookback)
        
        if cached.empty:
            logger.warning("Cached ATR series is empty")
            return 0.0
            
        atr_value = float(cached.iloc[-1])
        
        # Validate ATR value
        if pd.isna(atr_value) or atr_value < 0:
            logger.warning(f"Invalid ATR value calculated: {atr_value}")
            return 0.0
