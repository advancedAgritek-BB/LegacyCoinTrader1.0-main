...
class SignalFusionEngine:
    ...
    def fuse(self, df: pd.DataFrame, config: dict | None = None) -> Tuple[float, str]:
        ...
        # Apply sentiment weight reduction if configured
        sentiment_weight_multiplier = config.get("sentiment_weight_multiplier", 0.5) if config else 0.5
        ...
        for fn, weight in self.strategies:
            ...
            # Reduce weight for sentiment-related strategies
            if "sentiment" in fn.__name__.lower():
                w *= sentiment_weight_multiplier
                # logger.debug(f"Reduced sentiment strategy weight for {fn.__name__}: {w:.3f}") # This line was commented out in the original file, so it's commented out here.
