...
def get_exchange(config) -> Tuple[ccxt.Exchange, Optional[KrakenWSClient]]:
    ...
    if exchange_name == "coinbase":
        # Initialize exchange - for dry run, we can use empty credentials for public data
        ...
        if coinbase_key:
            exchange_config["apiKey"] = coinbase_key
        if coinbase_secret:
            exchange_config["secret"] = coinbase_secret
        if coinbase_passphrase:
            exchange_config["password"] = coinbase_passphrase

        exchange = ccxt_mod.coinbase(exchange_config)

        # In dry run mode, we don't need authentication to work
        # So we'll catch and log any auth errors but continue
        if not dry_run and (coinbase_key or coinbase_secret):
            try:
                # Test the connection briefly
                exchange.load_markets()
            except Exception as e:
                logger.warning("Exchange authentication test failed: %s", e)
        ...
        exchange = ccxt_mod.kraken(exchange_config)

        # In dry run mode, we don't need authentication to work
        # So we'll catch and log any auth errors but continue
        if not dry_run and (api_key or api_secret):
            try:
                # Test the connection briefly
                exchange.load_markets()
            except Exception as e:
