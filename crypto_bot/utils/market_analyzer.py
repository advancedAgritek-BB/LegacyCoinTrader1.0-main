...
async def analyze_symbol(
    ...
    if regime != "unknown":
        ...
        # Apply sentiment weight reduction to ensure sentiment is not a deciding factor
        sentiment_weight_multiplier = config.get("sentiment_weight_multiplier", 0.3)  # Reduce sentiment impact by 70%
        ...
        # Apply sentiment boost if available, but with reduced impact
        if "sentiment_boost" in candidate:
            sentiment_boost = candidate["sentiment_boost"]
            if sentiment_boost > 1.0:
                # Apply very conservative sentiment boost
                conservative_boost = 1.0 + (sentiment_boost - 1.0) * sentiment_weight_multiplier
                final *= conservative_boost
                logger.debug(f"Applied conservative sentiment boost {conservative_boost:.3f} to {sym}")
