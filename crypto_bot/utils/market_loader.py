...
def _is_valid_base_token(token: str) -> bool:
    ...
    return is_valid_solana_token(token)
...
async def fetch_ohlcv_async(
    ...
    # Early validation: check if this symbol should be queried on this exchange
    exchange_id = getattr(exchange, "id", "unknown")
    if not is_symbol_supported_on_exchange(symbol, exchange_id):
        logger.warning(
            "Symbol %s is not supported on exchange %s (likely a Solana chain symbol)",
            symbol,
            exchange_id
        )
...
async def fetch_dex_ohlcv(
    ...
    *,
    ...
    if get_symbol_data_source(symbol, getattr(exchange, "id", "unknown")) == "cex":
