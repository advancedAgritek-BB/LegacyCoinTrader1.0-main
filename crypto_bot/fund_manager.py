...
async def auto_convert_funds(
    ...
    if dry_run:
        # Update paper wallet for dry run conversions
        if paper_wallet:
            try:
                # Simulate the conversion as a trade
                # We'll treat this as selling from_token and buying to_token
                trade_id = paper_wallet.open(f"{from_token}/{to_token}", "sell", amount, 1.0)  # Use 1.0 as placeholder price
                logger.info(f"Paper conversion opened: sell {amount} {from_token} for {to_token}, balance: ${paper_wallet.balance:.2f}")
                
                # Close the position immediately to simulate the conversion
                pnl = paper_wallet.close(f"{from_token}/{to_token}", amount, 1.0)
                logger.info(f"Paper conversion completed: PnL: ${pnl:.2f}, balance: ${paper_wallet.balance:.2f}")
            except Exception as e:
                logger.error(f"Failed to update paper wallet for conversion: {e}")
